	...
	/**
	 * Sets the "id" attribute of the specified element. This ID will disappear once
	...
	/**
	 * Clicks on a link, button .... If the click action causes a new page to load
	...
	/**
	 * Simulates the user clicking the "close" window or tab.
	 */
	...
	SynchronousWebDriver.prototype.doCreateCookie = function(nameValuePair,
			optionsString) {
		...
		if (results) {
			var expireDateInMilliseconds = (new Date()).getTime() + results[1]
					* 1000;
			cookie += "; expires="
					+ new Date(expireDateInMilliseconds).toGMTString();
		}
	 ...
	 * click action causes a new page to load, call waitForPageToLoad.
	 * 
	 * @param locator an element locator
	 * 
	 */
	...
	/**
	 * Drags an element and drops it on another element locatorOfObjectToBeDragged -
	 * an element to be dragged locatorOfDragDestinationObject - an element whose
	...
	SynchronousWebDriver.prototype.doDragAndDropToObject = function(locatorOfObjectToBeDragged, locatorOfDragDestinationObject) {
		...
		var destinationLeftX = this
				.getElementPositionLeft(locatorOfDragDestinationObject);
		var destinationTopY = this
				.getElementPositionTop(locatorOfDragDestinationObject);
		var destinationWidth = this.getElementWidth(locatorOfDragDestinationObject);
		var destinationHeight = this
				.getElementHeight(locatorOfDragDestinationObject);
	 ...
	 * an element locator
	 * @param keySequence
	 * Either be a string("\" followed by the numeric keycode of the key
	 * to be pressed, normally the ASCII value of that key), or a single
	 * character.
	 */
	SynchronousWebDriver.prototype.doKeyDown = function(locator, keySequence) {
	
		var element = findElement_(this.driver, locator, this.context)[1];
		triggerKeyEvent(element, 'keydown', keySequence, true, this.controlKeyDown,
				this.altKeyDown, this.shiftKeyDown, this.metaKeyDown);
	}
	...
	SynchronousWebDriver.prototype.doKeyUp = function(locator, keySequence) {
		var element = findElement_(this.driver, locator, this.context)[1];
		triggerKeyEvent(element, 'keyup', keySequence, true, this.controlKeyDown,
				this.altKeyDown, this.shiftKeyDown, this.metaKeyDown);
	...
	/**
	 * Press the meta key and hold it down until doMetaUp() is called or a new page
	 * is loaded.
	 * 
	 */
	...
	SynchronousWebDriver.prototype.triggerMouseEvent_ = function(element,
			eventType, clientX, clientY) {
		var event = element.ownerDocument.createEvent("MouseEvents");
		var view = element.ownerDocument.defaultView;
	
		event.initMouseEvent(eventType, true, true, view, 1, 0, 0, clientX,
				clientY, false, false, false, false, 0, element);
	...
	/**
	 * Remove a selection to the set of selected options in a multi-select element
	 * using an option locator.
	 * 
	 * @param locator
	 ...
	 */
	SynchronousWebDriver.prototype.doRemoveSelection = function(locator,
			optionLocator) {
		var element = findElement_(this.driver, locator, this.context)[1];
		var elementId = findElement_(this.driver, locator, this.context)[0];
	 ...
	 * 
	 * @param selectLocator
	 *            an identifying a drop-down menu
	 * @param optionLocator
	 *            an option locator
	 * 
	 */
	SynchronousWebDriver.prototype.doSelect = function(selectLocator,
			optionLocator, wasSelected) {
	...
	/**
	 * Configure the mouse speed If the mouse speed is negative,it is given the
	 * default mouse speed(10)
	 * 
	 * @param pixels
	...
	SynchronousWebDriver.prototype.doWindowFocus = function() {
		var script = new Array("window.focus()");
		this.driver.executeScript(script);
	}
	...
	SynchronousWebDriver.prototype.doWindowMaximize = function() {
		var script = new Array("if (window.screen) { window.moveTo(0, 0); window.resizeTo(window.screen.availWidth, window.screen.availHeight);};");
		this.driver.executeScript(script);
	}
...
// ======================================== ACCESSORS ==========================
	...
	SynchronousWebDriver.prototype.getAllButtons = function() {
		...
		for (var input in allInputs) {
			...
			if (("button" == type) || ("submit" == type) || ("reset" == type)) {
				var att = this.driver.getAttribute(true, allInputs[input],
						attributeId);
				buttons.push(att);
			}
		}
	...
	SynchronousWebDriver.prototype.getAllFields = function() {
		...
		for (var input in allInputs) {
			type = this.driver.getAttribute(true, allInputs[input], attributeType);
			if ("text" == type) {
				var fld = this.driver.getAttribute(true, allInputs[input],
						attributeId);
				fields.push(fld);
			}
		}
	...
	SynchronousWebDriver.prototype.getAllLinks = function() {
		...
		for (var id in elementAttribute) {
			var lk = this.driver
					.getAttribute(true, elementAttribute[id], attribute);
			links.push(lk);
		}
		return links;
	}
	...
	/**
	 * Gets the value of the cookie with the specified name, or throws an error if
	...
	SynchronousWebDriver.prototype.getEval = function(script) {
		var value = new Array("return (" + script + ");");
	...
	/**
	 * Gets all option indexes for selected options in the specified select or
	 * multi-select element.
	 * 
	 * @param selectLocator an element locator identifying a drop-down menu
	 ...
	 * 
	 * @param name the name of the cookie
	 * @return boolean true if a cookie with the specified name is present, or false
	 * otherwise.
	 */
	...
	/**
	 * Check if these two elements have same parent and are ordered siblings in the
	 * DOM.
	 * 
	 ...
	 * @return boolean true if element1 is the previous sibling of element2, false
	 * otherwise
	 */
	...
	function findElement_(driver, locator, context) {
		 ...
		 * 
		 * @param {Response}
		 *            respond Object to send the command response with.
		 * @param {Array
		 *            <string>} parameters A two-element array: the first element
		...
		switch (strategyName) {
			...
			case "link" :
				...
				if (use.indexOf("exact:") == 0) {
					...
					res = makeSimpleFinder(ElementLocator.LINK_TEXT, use
									.substring(6));
					 ...
					 * 
					 * @param {Response}
					 *            response Object to send the command response with.
					 * @param {Array.
					 *            <string>} parameters A two-element array: the
					 *            first element should be the type of locator
					 *            strategy to use, the second is the target of the
					 *            search.
					 */
	...
	SynchronousWebDriver.prototype.makePageLoadCondition = function(timeout) {
		...
		return function() {
			...
			switch (state) {
				case "retry" :
					 var script = new Array("return(!document.old && 'complete' == document.readyState)");
