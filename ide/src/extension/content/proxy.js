...
function FirefoxDriver() {
	...
	// get an enumerator which iterates over all windows from the oldest window
	// to the youngest
...
var withoutElementIdMethod = ["getCurrentWindowHandle", "get", "getCurrentUrl",
		"close", "getPageSource", "findChildElements", "findElements",
		"goBack", "refresh", "title", "addCookie", "getCookie", "findElement",
		"executeScript", "setMouseSpeed", "getMouseSpeed", "addCookie",
		"deleteCookie", "deleteAllCookies", "getMouseSpeed",
		"getScreenshotAsBase64()"];

var withElementIdMethod = ["click", "getText", "getTagName", "submit",
		"getValue", "setSelected", "sendKeys", "getTagName", "getAttribute",
		"submit", "isSelected", "toggle", "dragElement", "getSize",
		"getLocation", "isDisplayed"];
	...
	for (method in this.driver) {
		if (typeof this.driver[method] == "function") {
			if (withoutElementIdMethod.indexOf(method) != -1) {
				...
				var f = this.createNewFunctionWithoutID(realMethod,
						this.driver, this.context, this.fxdriver.fxbrowser);
				this.driver[method] = f;
			}
			if (withElementIdMethod.indexOf(method) != -1) {
				var realMethod = this.driver[method];
				var f = this.createNewFunctionWithID(realMethod, this.driver,
						this.context, this.fxdriver.fxbrowser);
				this.driver[method] = f;
			}
		}
...
WebDriverProxy.prototype.createNewFunctionWithoutID = function(realF, object,
		context, fxbrowser) {
	return function() {
...
WebDriverProxy.prototype.createNewFunctionWithID = function(realF, object,
		context, fxbrowser) {
...
function switchToWindow(windowId, opt_searchAttempt) {
	...
	if (!windowFound) {
		...
		if (searchAttempt > 3) {
			...
			var self = this;
			this.wm.getMostRecentWindow('navigator:browser').setTimeout(
					"self.switchToWindow(windowId,(searchAttempt + 1))", 500);
		}
	}
